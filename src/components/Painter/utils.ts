...
const getColorByLabel = (label: string): Painter.Color =>
  randomColor({
    seed: label,
    format: "rgbArray",
    luminosity: "bright",
...
// 分类标签
export const drawClassLabel = (
  ctx: CanvasRenderingContext2D,
  classLabel: Painter.ClassLabel
) => {
  const px = 12;
  const py = 8;
...
export const drawLine = (
  ...
  ctx.beginPath();
  ctx.arc(sx, sy, lineWidth * 2, 0, 2 * Math.PI);
  ctx.fill();
  ctx.closePath();
...
export const drawCircle = (
  ...
  const { centre: { x: _x, y: _y, visible }, radius, color: [r, g, b] } = circle
