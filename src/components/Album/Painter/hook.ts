...
import { drawBBox, drawClassLabel, drawLine } from './utils'
...
const compositionImage = async (ctx: CanvasRenderingContext2D, meta: Album.ImgMeta, raw = false) => {
  const { src, bBoxes, rawSrc, classLabel, lines } = meta
...
export const usePainter = (props: Album.ImgMeta) => {
  ...
  React.useEffect(
    () => {
      ...
      setRawLoading(true)
      compositionImage(ctx, props, true)
        .then(() => {
          setRawLoading(false)
        })
    },
