...
export const deviceListDataAtom = atom(async (get) => {
  ...
  const device_chip_id = get(selectedChipAtom)
  if (!device_chip_id || (!group && group !== 0)) {
    return {
      total: 0,
      items: [],
    }
  ...
  const { success, data } = await groupDeviceAPI.list(group, { ...filter, device_chip_id })
...
export const useResetDeviceStore = () => {
  const setGroup = useSetAtom(_selectedDeviceGroupAtom)
  const setFlag = useSetAtom(flagAtom)
  const setGroupSelectorRef = useSetAtom(groupSelectorRefAtom)

  return () => {
    setGroup(null)
    setFlag(false)
    setGroupSelectorRef(null)
  }
}