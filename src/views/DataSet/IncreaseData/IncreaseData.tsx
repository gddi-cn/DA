...
const IncreaseData = (): JSX.Element => {
  ...
  const timeRef = useRef<any>({
    pre: 0, next: 0, preLoad: 0, nextLoad: 0
  })
  ...
  const handleBack = () => {
    ...
    navigate(APP_DATASET_DETAIL)
  }


  const handleCnasel = useDebounceFn(async () => {
  ...
  const handleOnuploadBigData = async (file: File | undefined) => {
    ...
    try {
      ...
      const createParams: any = {
        Body: file,
        Bucket: additional?.allowed_bucket[0], // Key: hash,
        ACL: 'public-read',
        processCallback: (obj: any) => {
          const pre = timeRef.current.next
          const preLoad = timeRef.current.nextLoad
          timeRef.current = {
            pre: pre, next: new Date().valueOf() / 1000, preLoad: preLoad, nextLoad: obj.loaded
          }
  ...
  const uploadview = useMemo(() => {
    return (<UploadingView
      fileInfo={fileInfo} checking={checking} percent={percent} timeRef={timeRef}
      handleCnasel={handleCnasel.run}></UploadingView>)
  ...
  React.useEffect(() => {
    setExampleUrl(datasetType ? datasetSceneExampleURLMapping.get(datasetType as DatasetScene) || '' : '')
  }, [datasetType])

  return (<div styleName="IncreaseData" id="IncreaseData">
        ...
        <div className="form_content">
          {
            isUploading
              ...
              : (
                <UploadFile
                  hasPreview={true}
                  tips="支持.zip、tar、gz格式 "
                  regExp={regExp}
                  onUpload={handleOnuploadBigData}
                />
              )
          }
        </div>
