...
export const useFooter = () => {
  ...
  const handleClick = () => {
    ...
    const { train_set, val_set, scene } = dataset as Dataset

    if (train_set.class_count !== val_set.class_count && scene !== DatasetScene.CowFaceRecognition) return notification.error({
      message: '提示',
      duration: 3,
