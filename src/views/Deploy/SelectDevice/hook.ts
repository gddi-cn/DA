...
export const useRefreshDeviceList = () => {
  ...
  return async (firstPage?: boolean) => {
    ...
    const { success, data } = await deviceGroupAPI
      .fetchGroupDeviceList(
        groupId,
        {
          ...
          device_chip_id: selectedApp?.device_chip_id,
          sort,
          sort_by,
        }
      )
    ...
    const getKeepItems = (old: Array<Device.Instance['id']>) => {
      const newItems = old.filter(o => data.items?.some(x => x.id === o))
      return newItems
    }
    setSelectedDeviceIdList(getKeepItems)
  }
}
