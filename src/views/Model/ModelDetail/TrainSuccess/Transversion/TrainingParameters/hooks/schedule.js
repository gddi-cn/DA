...
channel.port2.onmessage = function () {
  ...
  const timeRema = timeRemaining()
  console.log('收到信息', timeRema)
  if (timeRema > 2 && timeRema < 16.66) {
    // 执行回调并把参数传给回调
...
export const requestIdleCallback = function (callback) {
  requestAnimationFrame(rafTime => {
    // 算出当前帧的结束时间 这里就先按照16.66ms一帧来计算
    frameDeadline = getCurrentTime() + 16.66
    // 存储回调
    penddingCallback = callback
    // 这里发送消息，MessageChannel是一个宏任务，也就是说上面onmessage方法会在当前帧执行完成后才执行
    // 这样就可以计算出当前帧的剩余时间了
    channel.port1.postMessage('timeline') // 发送内容随便写了
  })
}
