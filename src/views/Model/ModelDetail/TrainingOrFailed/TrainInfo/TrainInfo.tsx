...
const TrainInfo = (): JSX.Element => {
  ...
  const getTime = () => {
    ...
    if (status === 2) {
      ...
      valueList = [// `${0}元`,
        h + '小时' + m + '分钟', moment(+begin * 1000).format('YYYY-MM-DD'), moment(+begin * 1000).format('HH:mm:ss')]
    } else if (status === 3) {
      valueList = [// `${0}元`,
        '-', moment(+begin * 1000).format('YYYY-MM-DD'), moment(+begin * 1000).format('HH:mm:ss')]
    } else if (status === 6) {
      valueList = [// `${0}元`,
        '-', '-', '-']
    } else {
  ...
  const getConfig = () => {
    try {
      ...
      const {app_data_args, gpu_count, platform, mode, channel, resolution} = versionInfo.iter
      const platformlist = JSON.parse(platform)
      const {fps} = JSON.parse(app_data_args)

      return [...(platformlist as any[]), fps, gpu_count, mode, channel, resolution]
    } catch (e) {
      return [// `${0}元`,
        '-', '-', '-', '-', '-', '-', '-', '-']
    }
  ...
  const [menufacture, series, chip_type, fps, gpu_count, mode, channel, resolution] = getConfig()

  const text: string[] = ['自定义', '精度优先', '速度优先', '自定义']

  return (<div styleName="TrainInfo">
